...
Stable `v2.0.0` is blocked until the exact RC JAR passes all of the following on PlexonCraft:
...
Record exact RC SHA, JAR SHA-256, server build, Core build, migration backup path, test evidence and Spark comparison before stable promotion.
