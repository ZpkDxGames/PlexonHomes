...
public final class SafeTeleportService {
    private static final Set<Material> HAZARDS = Set.of(Material.LAVA, Material.FIRE, Material.SOUL_FIRE, Material.CACTUS,
    ...
    public SafeTeleportService(JavaPlugin plugin, Supplier<Snapshot> config) { this.plugin = plugin; this.config = config; }
    ...
    public CompletableFuture<Optional<Location>> resolve(Home home) {
        ...
        if (!structurallyValid(home)) return CompletableFuture.completedFuture(Optional.empty());
        World world = Bukkit.getWorld(home.worldId());
        if (world == null && home.worldName() != null) world = Bukkit.getWorld(home.worldName());
        if (world == null || !config.get().canTeleportTo(world)) return CompletableFuture.completedFuture(Optional.empty());
        Location base = home.toLocation(world);
        int chunkX = base.getBlockX() >> 4; int chunkZ = base.getBlockZ() >> 4;
        if (world.isChunkLoaded(chunkX, chunkZ)) return CompletableFuture.completedFuture(findSafe(base));
        World destinationWorld = world;
        return world.getChunkAtAsync(chunkX, chunkZ, true).thenCompose(chunk -> onMain(() -> findSafe(home.toLocation(destinationWorld))));
    }

    public boolean structurallyValid(Home home) {
        return home != null && home.ownerId() != null && home.homeId() != null && home.worldId() != null
                && finite(home.x()) && finite(home.y()) && finite(home.z()) && Float.isFinite(home.yaw()) && Float.isFinite(home.pitch())
                && home.worldName() != null && !home.worldName().isBlank() && home.revision() >= 1L;
    }
    ...
    public boolean isSafe(Location location) {
        ...
        if (!finite(location.getX()) || !finite(location.getY()) || !finite(location.getZ()) || !Float.isFinite(location.getYaw()) || !Float.isFinite(location.getPitch())) return false;
        ...
        return occupiable(feet) && occupiable(head) && support.getType().isSolid();
    }
    ...
    private Optional<Location> findSafe(Location base) {
        ...
        for (int dy = 1; dy <= snapshot.verticalRadius(); dy++) {
            Location up = base.clone().add(0, dy, 0); if (isSafe(up)) return Optional.of(up);
            Location down = base.clone().add(0, -dy, 0); if (isSafe(down)) return Optional.of(down);
        }
        for (int radius = 1; radius <= snapshot.horizontalRadius(); radius++) {
            for (int dx = -radius; dx <= radius; dx++) for (int dz = -radius; dz <= radius; dz++) {
                ...
                for (int dy = -snapshot.verticalRadius(); dy <= snapshot.verticalRadius(); dy++) {
                    Location candidate = base.clone().add(dx, dy, dz); if (isSafe(candidate)) return Optional.of(candidate);
                }
            }
        }
    ...
    private static boolean occupiable(Block block) {
        Material material = block.getType(); return material == Material.WATER || (block.isPassable() && material != Material.LAVA);
    }
    ...
    private <T> CompletableFuture<T> onMain(Supplier<T> supplier) {
        ...
        Bukkit.getScheduler().runTask(plugin, () -> { try { result.complete(supplier.get()); } catch (Throwable t) { result.completeExceptionally(t); } });
        return result;
    }
