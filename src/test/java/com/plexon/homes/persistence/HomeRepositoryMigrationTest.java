...
class HomeRepositoryMigrationTest {
    ...
    @Test void freshDatabaseStartsAtSchemaTwo() {
        try (var repository = new HomeRepository(temp.resolve("fresh.db"))) {
            repository.ready().join();
            assertEquals(2, repository.schemaVersion());
            assertTrue(repository.loadHomes(UUID.randomUUID()).join().isEmpty());
        }
    }
    ...
    @Test void migratesPublishedV1ShapeAndPreservesFields() throws Exception {
        ...
        try (var connection = DriverManager.getConnection("jdbc:sqlite:" + db); var statement = connection.createStatement()) {
            ...
            statement.execute("CREATE TABLE players(player_uuid TEXT PRIMARY KEY,last_name TEXT,updated_at INTEGER)");
            ...
            try (var ps = connection.prepareStatement("INSERT INTO homes VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)")) {
                ...
                ps.setDouble(6, 1.25); ps.setDouble(7, 70); ps.setDouble(8, -3.5); ps.setFloat(9, 90); ps.setFloat(10, 5);
                ps.setLong(11, 10); ps.setLong(12, 20); ps.setLong(13, 7); ps.executeUpdate();
            }
        }
        try (var repository = new HomeRepository(db)) {
            repository.ready().join();
            assertEquals(2, repository.schemaVersion());
            var homes = repository.loadHomes(owner).join();
            assertEquals(1, homes.size());
            Home migrated = homes.getFirst();
            assertEquals(HomeIdentity.legacyStableId(owner, "home"), migrated.homeId());
            assertEquals("home", migrated.nameKey());
            assertEquals("Home", migrated.displayName());
            assertEquals(world, migrated.worldId());
            assertEquals(7, migrated.revision());
        }
        assertTrue(java.nio.file.Files.isDirectory(temp.resolve("backups")));
        try (var backups = java.nio.file.Files.list(temp.resolve("backups"))) {
            assertTrue(backups.anyMatch(path -> path.getFileName().toString().startsWith("homes-schema-v1-")));
        }
    }

    @Test void v2RestartIsIdempotent() throws Exception {
        Path db = temp.resolve("restart.db");
        UUID owner = UUID.randomUUID();
        Home home = new Home(owner, UUID.randomUUID(), "mine", "Mine", UUID.randomUUID(), "world", 1, 65, 2, 0, 0, 1, 1, 1);
        try (var first = new HomeRepository(db)) { first.ready().join(); first.upsert(home).join(); }
        try (var second = new HomeRepository(db)) {
            second.ready().join();
            assertEquals(home.homeId(), second.loadHomes(owner).join().getFirst().homeId());
            assertEquals(2, second.schemaVersion());
        }
    }

    @Test void futureSchemaFailsClosedWithoutCreatingTables() throws Exception {
        Path db = temp.resolve("future.db");
        try (var connection = DriverManager.getConnection("jdbc:sqlite:" + db); var statement = connection.createStatement()) {
            statement.execute("CREATE TABLE migration_meta(key TEXT PRIMARY KEY,value TEXT NOT NULL)");
            statement.execute("INSERT INTO migration_meta VALUES('schema_version','99')");
        }
        try (var repository = new HomeRepository(db)) {
            CompletionException error = assertThrows(CompletionException.class, () -> repository.ready().join());
            assertTrue(error.getCause().getMessage().contains("initialize homes database"));
        }
        try (var connection = DriverManager.getConnection("jdbc:sqlite:" + db); var statement = connection.createStatement()) {
            ...
            try (var rs = statement.executeQuery("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='players'")) {
                assertTrue(rs.next());
    ...
    @Test void duplicateNameForDifferentStableIdsIsRejected() {
        Path db = temp.resolve("unique.db");
        UUID owner = UUID.randomUUID(), world = UUID.randomUUID();
        ...
        try (var repository = new HomeRepository(db)) {
            repository.ready().join();
            repository.upsert(firstHome).join();
            assertThrows(CompletionException.class, () -> repository.upsert(secondHome).join());
        }
    }

    @Test void malformedNonFiniteRecordIsRejectedBeforeWrite() {
        Path db = temp.resolve("malformed.db");
        UUID owner = UUID.randomUUID();
        Home bad = new Home(owner, UUID.randomUUID(), "home", "Home", UUID.randomUUID(), "world",
                Double.NaN, 64, 1, 0, 0, 1, 1, 1);
        try (var repository = new HomeRepository(db)) {
            repository.ready().join();
            assertThrows(CompletionException.class, () -> repository.upsert(bad).join());
        }
    }
}
